...
import {
  formatDateShort,
  getDatesInRange,
} from "../sp-dashboard-src/utils/date.js";
...
import {
  updateBarChart,
  updatePieChart,
} from "../sp-dashboard-src/rendering/charts.js";
...
describe("Date Range Reporter UI", () => {
  ...
  describe("Dashboard State Updates", () => {
    ...
    it("should not double-count time from parent aggregator tasks", () => {
      const today = new Date().toISOString().split("T")[0];
      const mockTasks = [
        makeTask("parent", "Parent Task", {
          subTaskIds: ["child"],
          timeSpentOnDay: { [today]: 7200000 },
        }),
        makeTask("child", "Child Task", {
          parentId: "parent",
          timeSpentOnDay: { [today]: 7200000 },
        }),
      ];
    ...
    it("should not mark a task overdue/late if dueDay is added on the same day after completion", () => {
      const now = Date.now();
      const task = makeTask("t-add-today", "Added Today", {
        isDone: true,
        doneOn: now,
      });
      const tasks = [task];
    ...
    it("should count tasks due today in totalTasks denominator even with no time logged", () => {
      ...
      const taskDueToday = makeTask("t-due-no-time", "Due Today No Time", {
        dueDay: todayStr,
      });
  ...
  describe("Navigation & Interactivity", () => {
    ...
    it("bar and pie charts should render for overdue and late types and details show badges", () => {
      ...
      const overdueTask = makeTask("t1", "Foo", {
        dueDay: "2026-02-20",
        timeSpentOnDay: { "2026-02-20": 0 },
      });
      const lateTask = makeTask("t2", "Bar", {
        isDone: true,
        doneOn: now,
        dueDay: yesterdayStr,
      });
    ...
    it("detail list columns are sortable when headers are clicked", () => {
      const taskA = makeTask("a", "A", {
        dueDay: "2026-01-01",
        timeSpentOnDay: { "2026-01-01": 3600000 },
      });
      const taskB = makeTask("b", "B", {
        dueDay: "2026-01-02",
  ...
  describe("Daily Breakdown Tab", () => {
    ...
    it("aggregates time per (date, project) pair, dropping empty-day cells", () => {
      ...
      const rows = Array.from(
        document.querySelectorAll("#daily-breakdown-body tr"),
      );
    ...
    it("Date → Project sort emits a Day total row between date groups, sorted desc", () => {
      ...
      document
        .getElementById("daily-breakdown-sort")
    ...
    it("Project → Date sort emits a Project total row and orders projects by total hours desc", () => {
      ...
      const tasks = [
        ...
        makeTask("b", "B", {
          projectId: "p2",
          timeSpentOnDay: { "2026-02-21": 7200000, "2026-02-22": 3600000 },
        }),
      ];
      const projects = [
        { id: "p1", title: "Alpha" },
    ...
    it("hours cell exposes decimal hours via title attribute", () => {
      ...
      const tasks = [
        makeTask("a", "A", {
          projectId: "p1",
          timeSpentOnDay: { "2026-02-20": 9000000 },
        }),
      ];
      const projects = [{ id: "p1", title: "Alpha" }];
      processData(tasks, projects);
    ...
    describe("Rounding", () => {
      ...
      it("changing rounding select re-renders cells with rounded values", () => {
        ...
        const tasks = [
          makeTask("a", "A", {
            projectId: "p1",
            timeSpentOnDay: { "2026-02-20": 135 * 60000 },
          }),
        ];
        const projects = [{ id: "p1", title: "Alpha" }];
        processData(tasks, projects);
        ...
        const cell = () =>
          document.querySelector("#daily-breakdown-body .time-cell");
        expect(cell().textContent.trim()).toBe("2h 15m");

        const roundingSelect = document.getElementById(
      ...
      it("decimal format combined with rounding renders rounded decimal", () => {
        ...
        document
          .getElementById("daily-breakdown-rounding")
          .dispatchEvent(new Event("change"));
        document.getElementById("daily-breakdown-format").value = "decimal";
        document
          .getElementById("daily-breakdown-format")
          .dispatchEvent(new Event("change"));
